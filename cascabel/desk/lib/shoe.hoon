...
/+  sole
|%
::  +option: an autocomplete candidate.
::
::    Inlined from /lib/language-server-complete, which base at this
::    kelvin does not ship even though the stock shoe imports it.  Only
::    the type is needed here: +tab below is a no-op, since cascabel is
::    driven by pokes and has no vocabulary to complete against.
::
++  option
  |$  [item]
...
++  agent
  ...
  ++  on-poke
    |=  [=mark =vase]
    ^-  (quip card:agent:gall agent:gall)
    ::  %eval-command: programmatic execution, bypassing the keystroke
    ::  buffer.  This is the caderno kernel contract.
    ::
    ::    The caller must already hold a subscription to /sole/[ship]/[ses],
    ::    which is what registers the session.  Results come back as
    ::    %sole-effect facts on that subscription, and a %pro signals that
    ::    the command has finished.
    ::
    ::    Reuses the inner agent's own +command-parser and +on-command, so
    ::    a poked command and a typed one take exactly the same path.
    ::
    ?:  ?=(%eval-command mark)
    ...
    ::  +tab: no completion.
    ::
    ::    Stock shoe completes against /lib/language-server-complete,
    ::    which base does not ship at this kelvin.  Cascabel is a
    ::    call-and-response kernel, so Tab has nothing to offer and the
    ::    honest thing is to do nothing rather than vendor a library
    ::    from a different kernel version to power a feature nobody uses.
    ::
    ++  tab
      |=  pos=@ud
  ...
  ++  on-peek
    |=  =path
    ^-  (unit (unit cage))
    ::  /x/sole/sessions: the active session names, which is how caderno
    ::  discovers that this agent can serve as a kernel
    ?:  =(/x/sole/sessions path)
      =/  names=(list json)
        %+  turn  ~(tap in ~(key by soles))
        |=(=sole-id s+ses.sole-id)
      ``json+!>([%a names])
