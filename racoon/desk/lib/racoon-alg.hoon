...
::
::  SPEC A8 asked in advance for "one call, so a better algorithm
::  replaces it".  That call is +facz, and the better algorithm is the
::  DOOR'S SAMPLE rather than an import -- see +alg below.  So this
::  library still builds from Racoon alone, and van Hoeij is bound in
::  from the Baloon side by /lib/baloon-alg.  SPEC-QUESTIONS R4 records
::  both the measurement and why the sample beat the import.
::
/-  *racoon
...
=<  ~(. alg firr:zx)
|%
::    +alg:  the library, as a door over its recombination step
::
::  .fir takes a PRIMITIVE SQUAREFREE polynomial to its irreducible
::  factors over Z.  Two arms have exactly that contract already:
::  +firr:zx, which is Zassenhaus and is the default bound below, and
::  +factor:vh in /lib/vanhoeij, which is van Hoeij.
::
::  IT IS THE SAMPLE RATHER THAN AN IMPORT so that this library keeps
::  building from Racoon alone.  Binding it from outside also needs no
::  threading: +add, +mul, +div, and +root all reach factorization
::  through +make, and an arm calling another arm of its own door
::  resolves against that door's sample -- so setting it once at the
::  top reaches every arm underneath.
::
::      /+  al=racoon-alg          Zassenhaus; Racoon-only, the default
::      =/  fast  ~(. al factor:vh)     van Hoeij, where Baloon is in scope
::
::  /lib/baloon-alg is that second line as a file, so the fast path is
::  also a plain import and nobody has to remember to bind it.
++  alg
  |_  fir=$-(zol (list zol))
  +|  %types
  ...
  ::  the only arm that reads .fir.
  ::
  ::  This is +factor:zx's pipeline with its recombination step left
  ::  open: Yun's squarefree decomposition, then .fir per part.  Bound to
  ::  the default +firr:zx it IS +factor:zx, modulo the multiplicities
  ::  dropped below; bound to +factor:vh it is van Hoeij, which is 21x
  ::  faster at r = 16 and degenerates to +firr:zx below +lat-min, so
  ::  neither binding can be the slower choice on any input.
  ::
  ::  The squarefree half stays HERE rather than in the sample.  Yun is
  ::  not the part with two implementations, and the injected contract
  ::  is crisper for excluding it: primitive squarefree in, irreducible
  ::  factors out, which is what both +firr:zx and +factor:vh already
